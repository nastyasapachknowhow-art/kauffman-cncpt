...
export class ProductCard extends ProductCardLink {
  ...
  updatePrice(event) {
    ...
    const volumePricingNote = productPrice?.querySelector(`[ref='volumePricingNote']`);
    const newPriceElement = event.detail.data.html.querySelector(`product-price [ref='priceContainer']`);
    const newVolumePricingNote = event.detail.data.html.querySelector(`product-price [ref='volumePricingNote']`);

    if (newPriceElement && priceContainer) {
      morph(priceContainer, newPriceElement);
    }

    if (!newVolumePricingNote) {
      volumePricingNote?.remove();
    } else if (!volumePricingNote) {
      priceContainer?.insertAdjacentElement('afterend', /** @type {Element} */ (newVolumePricingNote.cloneNode(true)));
    } else {
      morph(volumePricingNote, newVolumePricingNote);
    }
  }
...
class SwatchesVariantPickerComponent extends VariantPicker {
  #handleCardVariantUrlUpdateHandler = () => this.#handleCardVariantUrlUpdate();
  ...
  connectedCallback() {
    ...
    this.addEventListener(ThemeEvents.variantUpdate, this.#handleCardVariantUrlUpdateHandler);
  }

  disconnectedCallback() {
    super.disconnectedCallback();
    this.removeEventListener(ThemeEvents.variantUpdate, this.#handleCardVariantUrlUpdateHandler);
  }
  ...
  variantChanged(event) {
    ...
    const availableCount = parseInt(clickedSwatch.dataset.availableCount || '0', 10);
    const firstAvailableVariantId = clickedSwatch.dataset.firstAvailableOrFirstVariantId;
    const isUnavailableWithAvailableAlternative =
      clickedSwatch.dataset.optionAvailable === 'false' && availableCount > 0 && firstAvailableVariantId;

    // For swatch inputs, check if we need special handling
    if (isSwatchInput && isUnavailableWithAvailableAlternative) {
